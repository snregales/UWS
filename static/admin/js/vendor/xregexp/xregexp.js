...
// Check for correct `exec` handling of nonparticipating capturing __groups
 ...
 * @returns {Boolean} Whether the object match the type.
 */
 ...
 *   <li>`s` - dot match all (aka singleline)
 ...
 * added wins.
 * @memberOf XRegExp
 * @param {RegExp} regex Regex object that match the new token.
...
/**
 * Retrieves the match from searching a string using a chain of regexes that successively search
 * within previous match. The provided `chain` array can contain regexes and objects with `regex`
 * and `backref` properties. When a backreference is specified, the named or numbered backreference
 * is passed forward to the next regex or returned.
 * @memberOf XRegExp
 * @param {String} str String to search.
 * @param {Array} chain Regexes that each search for match within preceding results.
 * @returns {Array} Matches by the last regex in the chain, or an empty array.
 * @example
 *
 * // Basic usage; match numbers within <b> tags
...
/**
 * Returns a new string with one or all match of a pattern replaced. The pattern can be a string
 ...
 * @returns {String} New string with one or all match replaced.
 * @example
 *
 ...
 * capturing __groups, backreferences are spliced into the result each time `separator` is matched.
 ...
 */
    fixed.exec = function (str) {
        ...
        if (match) {
            ...
            // nonparticipating capturing __groups
            ...
            // Fix browsers that increment `lastIndex` after zero-length match
        ...
        if (isType(replacement, "function")) {
            ...
            result = nativ.replace.call(str, search, function () {
                ...
                return nativ.replace.call(String(replacement), replacementToken, function ($0, $1, $2) {
                     ...
                     * - `$1` is an error if there are no capturing __groups.
                     * - `$10` is an error if there are less than 10 capturing __groups. Use `${1}0` instead.
                     * - `$01` is equivalent to `$1` if a capturing group exists, otherwise it's an error.
                     * - `$0` (not followed by 1-9), `$00`, and `$&` are the entire match.
                     * Native behavior, for comparison:
                     * - Backreferences end after 1 or 2 digits. Cannot use backreference to capturing group 100+.
                     * - `$1` is a literal `$1` if there are no capturing __groups.
                     * - `$10` is `$1` followed by a literal `0` if there are less than 10 capturing __groups.
 ...
 * Octals except \0 not followed by 0-9 and backreferences to unopened capture __groups throw an
 * error. Other match are returned unaltered. IE <= 8 doesn't support backreferences greater than
 * \99 in regex syntax.
 */
    add(/\\(\d+)/,
        function (match, scope) {
 ...
 * Required for support of named capturing __groups. Also adds explicit capture mode (flag n).
 */
    add(/\((?!\?)/,
        function () {
            if (this.hasFlag("n")) {
...
(function (XRegExp) {
    ...
    XRegExp.addUnicodePackage({
        ...
        // \p{Any} match a code unit. To match any code point via surrogate pairs, use (?:[\0-\uD7FF\uDC00-\uFFFF]|[\uD800-\uDBFF][\uDC00-\uDFFF]|[\uD800-\uDBFF])
 ...
 * @returns {Array} Array of match, or an empty array.
 * @example
 *
 * // Basic usage
 * var str = '(t((e))s)t()(ing)';
